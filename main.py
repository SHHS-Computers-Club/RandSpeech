...
def main():
    ...
    def getfile():
        ...
        shuffle(text)
        try:
            pool = abs(int(float(input('Word pool length? >>> '))))
        except:
            print('Invalid length, setting length to 3.')
            pool = 3
        if pool >= len(text): pool = len(text)-1
        return [text[:pool],pool]
    ...
    while not text and not pool:
        text, pool = getfile()

    wordcount = randint(60,100)
    speech = [text[randint(0,pool-1)],text[randint(0,pool-1)]]
    for i in range(wordcount-2):
        speech.append(text[randint(0,pool-1)])
